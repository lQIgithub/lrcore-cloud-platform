...
import com.lrcore.auth.service.SysPasswordService;
 ...
 * <p>
 * 真实部件：SocialAuthController / SocialLoginService / SysPasswordService（BCrypt 校验）/
 *          TokenService（HS512 JWT 生成）/ LrcoreAuthExceptionHandler /
...
class SocialLoginFlowTest {
    ...
    @Test
    @DisplayName("回调：已绑定用户 → 平台令牌（JWT 三段式 + refreshToken）")
    void callback_bound_user_gets_platform_token() {
        ...
        assertThat(token.get("accessToken").asText().split("\\.")).hasSize(3); // JWT 三段
        assertThat(token.get("refreshToken").asText()).isNotBlank();
    ...
    void bind_with_correct_password_binds_and_logs_in() {
        ...
        assertThat(bindResult.getData().get("accessToken").asText().split("\\.")).hasSize(3);
    ...
    /** 执行扫码回调（data 为 JsonNode：TokenDto 无 Jackson 反序列化构造器，按 JSON 字段断言）。 */
    ...
    @Import({SocialLoginService.class, SocialAuthController.class, SysPasswordService.class,
    ...
    static class TestApp {
        ...
        TokenService tokenService(RedisService redisService) {
            return new TokenService(redisService);
        }
    }
